...
export default ({ on, mounted, trigger, get, set, when }) => {
  ...
  mounted(() => {
    let lastCenter = (localStorage.center || "")
      .split(" ")
      .map((p) => parseFloat(p));
    ...
    if (localStorage.colleague) {
      new (L.Control.extend({
        onAdd: function () {
          return (
            <div class="leaflet-bar colleague">
      ...
      new (L.Control.extend({
        onAdd: function () {
          ...
          return (
            <div
              class="leaflet-control-locate leaflet-bar leaflet-control"
              onClick={home}
            >
    ...
    const aerialLayer = L.tileLayer.wms(
    ...
    new (L.Control.extend({
      onAdd: function () {
        ...
        return (
          ...
          >
            <a class="leaflet-bar-part leaflet-bar-part-single">
              <span class="smallicon" id="maptype">
                {isAerialView() ? "🗺️" : "✈️"}
              </span>
            </a>
          </div>
    ...
    on("!+* pos.elevation", (elevation) => {
      if (!elevation) return;
      l._marker
        .bindPopup(`${elevation.placename}: ${elevation.elevation}m`)
        .openPopup();
    });
    ...
    new (L.Control.extend({
      onAdd: function () {
        const e = (
          ...
          >
            <a class="leaflet-bar-part leaflet-bar-part-single">⛰️</a>
          </div>
        );
        e.hidden = true;
        on("!+* pos.gps", (gps) => (e.hidden = !gps));
        return e;
      },
    }))({ position: "bottomright" }).addTo(map);
    ...
    on("!+* map.$id", (mark, { path }) => {
      ...
      const markers = [L.marker([mark.lat, mark.lon], { icon })];
      ...
      on(`!+* ${path}.*`, () => {
        ...
        const popupContent = (
          <div>
            ...
            {Object.values(mark.trip || {}).map((v) => (
              <div>{v}</div>
            ))}
            {Object.values(mark.height || {}).map((v) => (
              <div>{v}</div>
            ))}
            {mark.specialText ? <i>{mark.specialText}</i> : null}
            <a class="more" onClick={(e) => showMore(e, mark)}>
              Vis mer
            </a>
          </div>
        );
