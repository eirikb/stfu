...
export default ({ on, get, set, merge }) => {
  function getColleagueId() {
    return localStorage.colleague?.substr(
      0,
      localStorage.colleague.indexOf(" ")
    );
  }
  ...
  on("= loadMark", async (path) => {
    ...
    const trip = tables[0]
      ? [...tables[0].querySelectorAll("tr")].slice(1).map((tr) => tr.innerText)
      : [];
    const height = tables[1]
      ? [...tables[1].querySelectorAll("tr")].slice(1).map((tr) => tr.innerText)
      : [];
  ...
  async function loadPoints() {
    const colleague = getColleagueId();
  ...
  async function loadSomeonesPoints(colleague) {
    ...
    if (links) {
      result = [...links].map((node) =>
        node.href.substring(node.href.lastIndexOf("/") + 1)
      );
    }
  ...
  async function loadMap() {
    ...
    const [map, dones] = await Promise.all([loadMapPoints(), loadPoints()]);
    set(
      "map",
      map.reduce((res, mark) => {
